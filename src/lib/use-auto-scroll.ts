...
export const INITIAL_SCROLL_STATE: ScrollState = { phase: 'scrolling', holdUntil: 0 }

/**
 * One frame of the venue-screen loop, kept pure so it can be tested without a
 * browser: creep down, land exactly on the bottom and hold, then jump to the
 * top and start over.
 */
export function nextScrollStep(
  ...
  if (state.phase === 'holding') {
    if (now >= state.holdUntil) {
      return { scrollTo: 0, state: { phase: 'scrolling', holdUntil: 0 } }
    }
    return { scrollTo: null, state }
  }

  const next = scrollY + SPEED_PX_PER_SEC * elapsedSec
  if (next >= maxScroll - 1) {
    return {
      scrollTo: maxScroll,
      state: { phase: 'holding', holdUntil: now + BOTTOM_PAUSE_MS },
    }
  }
  return { scrollTo: next, state }
}
...
export function useAutoScroll(active: boolean): void {
  useEffect(() => {
    ...
    const tick = (now: number): void => {
      ...
      const result = nextScrollStep(state, now, elapsedSec, window.scrollY, maxScroll)
      state = result.state
